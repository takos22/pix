...
const reconcileScoOrganizationLearnerManually = async function (request, h) {
...
const reconcileScoOrganizationLearnerAutomatically = async function (request, h) {
  const authenticatedUserId = request.auth.credentials.userId;
  const payload = request.payload.data.attributes;
  const campaignCode = payload['campaign-code'];
...
const generateUsername = async function (request, h) {
  const payload = request.payload.data.attributes;
  ...
  return h.response(scoOrganizationLearnerSerializer.serializeWithUsernameGeneration(scoOrganizationLearner)).code(200);
};
...
const createUserAndReconcileToOrganizationLearnerFromExternalUser = async function (request, h) {
...
const generateUsernameWithTemporaryPassword = async function (request, h) {
  const payload = request.payload.data.attributes;
  const organizationId = payload['organization-id'];
  const organizationLearnerId = payload['organization-learner-id'];
...
const checkScoAccountRecovery = async function (request, h) {
